 ...
 * @Autor: huizhang43
 * @Date: 2026-06-02
 * @Description: blog-sharon文章详情页面 - 格式自动切换 (HTML/Markdown) + TOC导航
-->
<template>
  <div class="blog-detail">
    ...
    <div v-else-if="error" class="state-wrap error-state">
      <p>文章不存在或已被删除</p>
      <router-link to="/blog" class="back-btn">← 返回文章列表</router-link>
    </div>
...
export default {
  ...
  methods: {
    fetchArticle() {
      ...
      this.API.getBlogSharonPostById(id, {}, { notify: false })
        .then(res => {
          if (!res.data) {
            this.error = true;
            this.loading = false;
    ...
    renderContent() {
      ...
      // Format switching: markdown → showdown conversion, html → direct render
      if (this.article.formatContent === 'markdown') {
        ...
        rawHtml = converter.makeHtml(this.article.content);
      } else {
        // HTML or any other format: render directly via v-html
        rawHtml = this.article.content;
      }

      // Sanitize: strip script/iframe/object/embed tags to mitigate XSS (T-05-03)
