        ...
        <el-form-item v-for="(formTtem, index) in formMap" :key="index" :label="formTtem.label" :prop="formTtem.key">
          ...
          <div class="icon-group" v-if="formTtem.type === 'icon'">
            <el-tabs v-model="activeName" type="border-card" @tab-click="handleTabClick">
              ...
              <el-tab-pane v-if="showFindFaviconTab" label="AI生成" name="query">
  ...
  methods: {
    ...
    queryFaviconbyUrl() {
      ...
      this.API.findAppFavicon({ url: this.formData['url'] }, { notify: false }).then(res => {
        const data = res.data || {};
        this.favicons = data.icons || [];
        // 仅当描述框为空时自动填充 AI 生成的网站介绍，已手填内容不覆盖
        const description = data.description;
        const current = this.formData['description'];
        if (description && (!current || !String(current).trim())) {
          this.$set(this.formData, 'description', description);
        }
      });
    },
