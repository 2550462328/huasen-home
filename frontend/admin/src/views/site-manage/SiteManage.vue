...
export default {
  ...
  methods: {
    // 消费表盘快捷入口设置的待执行动作（经 store 传递，避免路由 query 触发 tab 刷新销毁组件）
    consumePendingAction() {
      if (this.$store.state.pendingAction === 'add') {
    ...
    handleEdit(index, row) {
      ...
      this.$nextTick(async () => {
        ...
        let columnId = [];
        try {
          // 拓展数据处理（后端 expand 字段是 Map，序列化为 JSON 对象；老数据/新建场景可能是字符串）
          let expand = typeof row.expand === 'string' ? JSON.parse(row.expand || '{}') : (row.expand || {});
          siteTag = expand.tag || [];
          sitePin = expand.pin || [];
          let columnResult = await this.API.findSiteColmunByList({ siteId: row.id }, { notify: false });
    ...
    async save(form) {
      ...
      // 处理拓展字段（后端 expand 字段是 Map，编辑时拿到的是对象；新建时 form 默认是字符串 '{}'）
      let expand = typeof params.expand === 'string' ? JSON.parse(params.expand || '{}') : (params.expand || {});
      ...
      // 后端期望 expand 为 JSON 对象（Map），不要再 stringify
      params.expand = expand;
      // 添加/编辑站点数据
      let siteIds = [];
      if (this.mode === 'edit') {
        await this.API.updateSite(params);
        siteIds.push(params.id);
      } else if (this.mode === 'add') {
        delete params.id;
        delete params._v;
        let siteResult = await this.API.addSite(params);
        siteIds.push(siteResult.data.id);
      }
      ...
      try {
        ...
        sites.forEach(item => {
          if (params.siteTag.length) {
            ...
            // 后端 expand 字段是 Map，直接传对象（stringify 会被后端 instanceof Map 判断静默忽略）
            item.expand = expand;
          }
        });
        // 添加站点链接
        ...
        if (params.columnId.length) {
          let siteIds = siteResult.data.map(item => item.id);
